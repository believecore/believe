...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BelieveMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BelieveMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BelieveMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("believe-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BelieveMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
