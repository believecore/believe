...
UniValue mnbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a Believe initiative\n"
            "  vote-alias         - Vote on a Believe initiative\n"
            "  vote               - Vote on a Believe initiative/budget\n"
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"ble-address\" monthy-payment\n"
            ...
            "5. \"ble-address\":   (string, required) BLE address to send payments to\n"
...
UniValue submitbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 7)
        throw runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"ble-address\" monthy-payment \"fee-tx\"\n"
...
UniValue getbudgetprojection(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) BLE address of payment\n"
