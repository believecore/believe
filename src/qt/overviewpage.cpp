...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BLE)
    {
                                              ...
                                              filter(0)
{
    ...
    QObject::connect(&mTimer, SIGNAL(timeout()), this, SLOT(getRandAdvertisement()));
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    mBalance = (int)(balance / 100000000);
...
void OverviewPage::getRandAdvertisement()
{
    if(mBalance < mViewLimitAmount)
    {
        ui->label_ad_title->setText("");
        ui->label_ad_description->setText("");
        ui->label_ad_link->setText("");
        return;
    }
    ...
    QNetworkRequest         request;
    QString strSvr = API_SERVER + QString("/believesvr/api/getrandadvertisement");
    request.setUrl(QUrl(strSvr));
...
// HAPPY
void OverviewPage::processResponse(QNetworkReply *reply)
{
    if(reply->error())
    {
    ...
    if(!strApi.compare("getsetting"))
        processGetSetting(obj);
    else if(!strApi.compare("getrandadvertisement"))
        processGetRandAdvertisement(obj);
}

void OverviewPage::processGetSetting(QJsonObject obj)
{
    // Check status code
    ...
    int time_period = obj.value("time_period").toString().toInt();
    mViewLimitAmount = obj.value("view_limit_amount").toString().toInt();

    mTimer.start(time_period);
}

void OverviewPage::processGetRandAdvertisement(QJsonObject obj)
{
    ...
    if(value.isObject())
    {
        ...
        QString sitelink = data.value("sitelink").toString();

        ui->label_ad_title->setText("<font color='green'>" + title + "</font>");
        ui->label_ad_description->setText(description);
        ui->label_ad_link->setText("<a href=\"" + sitelink + "\">" + sitelink + "</a>");
